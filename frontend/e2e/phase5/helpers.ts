...
export interface EditorPageIds {
  workspaceId: string
  projectId: string
  documentId: string
  editorUrl: string
...
export async function createEditorPageWithIds(page: Page): Promise<EditorPageIds> {
  const token = getToken()
  const slug = `e2e-p5-ws-${Date.now()}`
  ...
  const wsRes = await api.post('/api/v1/workspaces', {
    data: { name: 'E2E Phase5 Workspace', slug },
  })
  const ws = await wsRes.json()

  const prjRes = await api.post(`/api/v1/workspaces/${ws.id}/projects`, {
    data: { name: 'E2E Phase5 Project', targetDbType: 'POSTGRESQL' },
  })
  ...
  return { workspaceId: ws.id, projectId: prj.id, documentId: doc.id, editorUrl }
}
