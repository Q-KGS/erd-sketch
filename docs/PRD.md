...
| 기존 도구 | 주요 한계 | ErdSketch 개선점 |
|-----------|-----------|------------------|
| **dbdiagram.io** | DBML 문법 학습 필요, 실시간 협업 미지원, 시각적 편집 불가 | 코드 없이 직관적 드래그&드롭 + CRDT 기반 실시간 협업 |
| **DrawSQL** | 무료 플랜 제한적(15테이블), 오프라인 미지원, 라이브 DB 동기화 없음 | 오프라인 지원(y-indexeddb), 자체 호스팅 가능 |
| **Lucidchart** | 범용 다이어그램 도구(DB 특화 X), DDL 생성/파싱 미지원, 고가 | DB 설계 특화, 다중 방언 DDL 생성/역공학 기본 제공 |
| **MySQL Workbench / pgModeler** | 특정 DB 전용, 데스크톱 전용, 실시간 협업 불가 | 웹 기반, 4개 DB 방언 동시 지원, 팀 협업 내장 |

**핵심 차별 포인트:**

1. **시각 편집 + 실시간 협업 통합** — 대부분 도구가 둘 중 하나만 지원
2. **다중 DB DDL 네이티브 지원** — 범용 도구에서 제공하지 않는 DDL 생성/파싱/마이그레이션
3. **셀프 호스팅 & 오프라인** — SaaS 의존 없이 팀 인프라에서 운영 가능
...
**시장 규모:**
- 데이터 시각화 도구 시장 약 $10.2B(2026년 기준)
- DBMS 시장 $92B(2026) → $173B(2032), CAGR 10.8%

**수요 트렌드:**
- 조직 내 데이터 모델링 활용률 2023년 51% → 2024년 64% (YoY 13%↑)
- 2026년까지 75% 조직이 시각적 모델링 도구 도입 전망

**시장 기회:**
- 기존 도구 대부분 SaaS 종속 → 셀프 호스팅 수요 존재 (보안 민감 조직)
- "시각 편집 + 실시간 협업 + DDL 네이티브" 3요소를 동시 충족하는 도구 부재
- AI 기반 스키마 자동 생성/최적화가 2026년 표준 기능화 전망 → 향후 확장 가능

**타겟 시장:**
- 중소규모 개발팀(5~50명)
- 보안 요구가 높은 기업 내부 팀
- DB 설계가 빈번한 SI/컨설팅 조직
