...
| 데이터베이스 | H2 파일 DB (`/data/erdsketch.mv.db`) |
| 볼륨 | `erdsketch_data` (1GB) — `/data` 마운트 |
...
spring:
  datasource: H2 파일 DB (/data/erdsketch)
  flyway.enabled: false          # Flyway 비활성화 (ddl-auto: update 사용)
  mail: SendGrid SMTP 설정

app:
  jwt.secret: ${JWT_SECRET}      # Fly secret에서 주입
  oauth.enabled: false           # OAuth 비활성화 (기본값)
...
fly ssh console --command "ls /data"  # 단일 명령 실행
```

### 볼륨 관리

```bash
fly volumes list                   # 볼륨 목록
fly volumes show erdsketch_data    # 볼륨 상세 정보
```
...
- PostgreSQL과 달리 단일 JVM만 접속 가능 (멀티 인스턴스 불가)
- `min_machines_running = 0` → 트래픽 없으면 머신이 자동 중지됨 (cold start ~20초)
- DB 데이터는 `/data` 볼륨에 영속 보관 — 머신 재시작/재배포에도 유지
- 볼륨이 삭제되면 모든 데이터 소실 — **볼륨 삭제 금지**
...
- Fly 환경에서 비활성화 (`flyway.enabled: false`)
- 스키마 변경은 `hibernate.ddl-auto: update`로 자동 적용
- 신규 NOT NULL 컬럼 추가 시 기존 데이터에 NULL이 채워지는 문제 주의
...
| 항목 | 로컬 | Fly.io |
|------|------|--------|
| DB | PostgreSQL (Docker) | H2 파일 DB |
| 마이그레이션 | Flyway | ddl-auto: update |
