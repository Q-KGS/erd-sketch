...
- [x] E2E-VER-01 (버전 복원) Playwright 자동화 완료 (`e2e/phase4/version.spec.ts`)
- [x] E2E-CMT-01 (댓글 + 답글 + 해결 + 필터) Playwright 자동화 완료 (`e2e/phase4/comment.spec.ts`)
- [x] E2E-DIFF-02 마이그레이션 DDL 실제 DB 실행 — 백엔드 통합 테스트(B-MIG-01~07)로 대체
- [x] E2E-OFFLINE-01 (오프라인 편집 → 재연결) Playwright `context.setOffline()` 자동화 완료 (`e2e/phase4/offline.spec.ts`)
- [x] E2E-PDF-01 PDF 내보내기 다운로드 Playwright 자동화 완료 (`e2e/phase4/pdf.spec.ts`)
