...
public class OracleDdlGenerator {
    ...
    public String generate(Map<String, Object> schema, List<String> tableIds, boolean includeDrops, List<String> warnings) {
        ...
        if (includeDrops) {
            tableValues.forEach(tableObj -> {
                ...
                sb.append("BEGIN\n")
                  .append("  EXECUTE IMMEDIATE 'DROP TABLE ").append(quoteIdentifier(name)).append(" CASCADE CONSTRAINTS';\n")
    ...
    private void generateTableDdl(StringBuilder sb, Map<String, Object> table, List<String> warnings) {
        ...
        checkIdentifierLength(tableName, warnings);
        ...
        for (Map<String, Object> col : columns) {
            ...
            checkIdentifierLength(colName, warnings);
    ...
    private void generateForeignKey(StringBuilder sb, Map<String, Object> rel, Map<String, Object> tables, List<String> warnings) {
        ...
        // Oracle supports ON DELETE CASCADE and ON DELETE SET NULL only
        if ("CASCADE".equals(onDelete)) {
            sb.append(" ON DELETE CASCADE");
        } else if ("SET_NULL".equals(onDelete)) {
    ...
    private void checkIdentifierLength(String name, List<String> warnings) {
        if (name != null && name.length() > MAX_IDENTIFIER_LENGTH) {
            warnings.add("Identifier too long (max " + MAX_IDENTIFIER_LENGTH + " chars for Oracle 11g): " + name);
        }
    }

    private String mapDataType(String type) {
        if (type == null) return "CLOB";
        return switch (type.toUpperCase()) {
            case "INT", "INTEGER" -> "NUMBER(10)";
            case "BIGINT" -> "NUMBER(19)";
            case "SMALLINT" -> "NUMBER(5)";
            case "SERIAL" -> "NUMBER(10)";
            case "BIGSERIAL" -> "NUMBER(19)";
            case "DECIMAL", "NUMERIC" -> "NUMBER";
            case "FLOAT", "REAL", "DOUBLE", "DOUBLE PRECISION" -> "NUMBER";
            case "BOOLEAN", "BOOL", "BIT" -> "NUMBER(1)";
            case "DATE" -> "DATE";
            case "DATETIME", "TIMESTAMP" -> "TIMESTAMP";
            case "TIMESTAMPTZ" -> "TIMESTAMP WITH TIME ZONE";
            case "UUID", "UNIQUEIDENTIFIER" -> "VARCHAR2(36)";
            case "VARCHAR", "VARCHAR2" -> "VARCHAR2(255)";
            case "CHAR" -> "CHAR";
            case "JSON", "JSONB" -> "CLOB";
            case "TEXT", "LONGTEXT", "NTEXT" -> "CLOB";
